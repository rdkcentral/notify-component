...
# Changing directory path based on folder structure
cd notify_comp
...
log "INFO" "Running make for notify_comp_gtest.bin..."
...
if [ -f "./source/test/notify_comp_gtest.bin" ]; then
    log "INFO" "Running notify_comp_gtest.bin"
    ./source/test/notify_comp_gtest.bin
    log "INFO" "Completed Test Execution"
else
    log "ERROR" "notify_comp_gtest.bin does not exist, cannot run tests"
    exit 1
fi
